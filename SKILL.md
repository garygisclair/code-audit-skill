...
| **Web (Angular)** | Angular components live in **module directories**, not individual files. Each module directory (e.g., `button/`, `dialog/`, `tabs/`) contains a `@Component`-decorated class with a separate `.html` template and `.scss` styles. **Discovery strategy:** (1) Find the component root (e.g., `src/material/`, `src/lib/`, `projects/components/src/`). (2) List subdirectories — each is a component module. (3) Exclude CDK/utility directories (`cdk/`, `core/`, `testing/`, `schematics/`). (4) For each module, find `*.ts` files containing `@Component({` (skip `*.spec.ts`, `*harness*`, `*module*`, `index.ts`, `public-api.ts`). (5) Count lines across the module's non-test `.ts` files. (6) Report by **module** (not individual class) — one row per directory. List the key exported classes in the "Exports" column. Angular modules often contain 3-10+ sub-components (e.g., Datepicker has MatDatepicker, MatCalendar, MatDateRangeInput, etc.) — group them under the module name. |
